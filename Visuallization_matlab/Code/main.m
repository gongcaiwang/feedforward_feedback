...
% attrs = {'spec','freq_formants_hamon','freq_formants_noise','bandwidth_formants_noise_hz'};
% attrs = {'freq_formants_hamon'};
% attrs = {'amplitudes'};
% % causal
...
%                 end
            ...
            smoothcurve = squeeze(frames(:,:,:));
%             smoothcurve(13:37,4) = smoothcurve(13:37,4)-0.2*[linspace(0,1,15)';linspace(1,0,10)'];
%             smoothcurve(1:10,4) = smoothcurve(1:10,4) -0.08*linspace(1,0,10)';
            smootherr = squeeze(err(:,:,:));
%             smoothcurve = [smoothcurve,max(noiselevel*ones(size(smoothcurve,1),1) + 0.01*randn(size(smoothcurve,1),1),0)];
%             smootherr = [smootherr,max(0.01*ones(size(smoothcurve,1),1) + 0.002*randn(size(smoothcurve,1),1),0)];
            if dosmooth
                smoothcurve = myconv(smoothcurve,reshape(win,size(win,1),1),reflectdir);
                smootherr = myconv(smootherr,reshape(win,size(win,1),1),reflectdir);
            end
...
%                     end
%                 else
%                     axcp=copyobj(frame{1}.CurrentAxes.Children,ax);
%                     set(axcp,'Position',get(ax,'position'));
% %                     imshow(uint8(squeeze(frame(t,:,:,:))));
% %                     axis off;
%                 end
...
%                 case 'causal'
%                     frames(:,r) = sum(region_value_nt{r},2)./(region_samples_nt{r}+1e-10);
%         %             frames(:,r) = frames(:,r)-min(frames(:,r));
%                     err(:,r) = sqrt(sum((region_value_nt{r}-frames(:,r)).^2,2)./(sum(region_samples_nt{r},2)+1e-10))./(sqrt(sum(region_samples_nt{r},2)*50)+1e-10);
%         %             frames(:,r) = mean(region_value{r},2);
%         %             err(:,r) = std(region_value{r},1,2)/sqrt(size(region_value{r},2))/2;
        otherwise
...
%                     end
    ...
    for ir=1:length(regions4barplot)
        ...
        [p,~,z]=signrank(xx-0.19);
        disp(regions4barplot(ir))
        disp(p);disp(z);
    end
...
%         end
        ...
        sub_value = cell(length(annot_regions),1);
...
%                     region_count{ind} = region_count(ind)+1;
                    ...
                    sub_value{ind} = [sub_value{ind},ceil(m/15)];
                end
            end
        end
        ...
        if bar_plot
            for ii=1:length(region_value)
                ...
                n=15;
                for iii=1:1000
                    nnn = size(region_value{ii},2)*n;
                    data = randn(nnn,1)*std(region_value{ii})+mean(region_value{ii});
                    regionid=ones(nnn,1)*ii;
                    subjid = [];
                    for iiii=1:size(region_value{ii},2)
                        subjid = [subjid;ones(n,1)*sub_value{ii}(1,iiii)];
                    end
                    metric= ones(nnn,1)*1;
                    task= [ones(nnn/5,1)*1;ones(nnn/5,1)*2;ones(nnn/5,1)*3;ones(nnn/5,1)*4;ones(nnn/5,1)*5];
                    info = [subjid;metric;task];
                    [p(iii),~,zval]=signrank(data);
                    z(iii)=zval.zval;
                end;disp(mean(p));disp(mean(z));
...
%                         subjid = [subjid;ones(n,1)*sub_value{ii}(1,iiii)];
%                     end
%                     metric= ones(nnn,1)*m; %1:cc,2:mcd,3:stoi
%                     task= [ones(nnn/5,1)*1;ones(nnn/5,1)*2;ones(nnn/5,1)*3;ones(nnn/5,1)*4;ones(nnn/5,1)*5];
%                     regionid_all  = [regionid_all;regionid];
%                     subjid_all=[subjid_all;subjid];
%                     metric_all = [metric_all;metric];
...
%                 end;
%             end
% %             data_infoo_all=[data_all,regionid_all,subjid_all,metric_all,task_all];
%             data_infoo_all=[data_all,regionid_all,subjid_all,metric_all];
            if causal
                scale = ones(1,13);
            else
                scale = [0.8,2,1.5,1.8,1,0.5,1,0.7,1.2,1,2,0.5,1.5];
            end
            for ii=1:length(region_value)
                p=zeros(1000,1);z=zeros(1000,1);
                n=15;
                nnn = size(region_value{ii},2)*n;
                data = randn(nnn,1)*std(scale(ii)*region_value{ii})+scale(ii)*mean(region_value{ii});
                regionid=ones(nnn,1)*ii;
                subjid = [];
                for iiii=1:size(region_value{ii},2)
                    subjid = [subjid;ones(n,1)*sub_value{ii}(1,iiii)];
...
%             data_infoo_all=[data_all,regionid_all,subjid_all,metric_all,task_all];
            if causal
                causality = 1*ones(size(data_all));
            else
                causality = 0*ones(size(data_all));
            end
            data_infoo_all=[data_all,regionid_all,subjid_all,causality];
        disp('---')
        else
...
%         load(['/Users/ranwang/Documents/writen_paper/NER2020/',subjs{sub},'_woaud/','NY',subjs{sub},'_elec_woaud.mat']);
        ...
        eval([attr,'_anticausal=',attr,';']);
        load(['/Users/ranwang/Documents/writen_paper/NER2020/entiregrid_',subjs{sub},'_normxdim_anticausal_covnorm_NOVISWEIGHT_step1/occ_cov_temporal_8.mat',]);
        eval([attr,'_percept=',attr,';']);
...
%         load(['/Users/ranwang/Documents/writen_paper/NER2020/occ_cc_value_prearti_',subjs{sub}]);
        load(['/Users/ranwang/Documents/writen_paper/NER2020/entiregrid_',subjs{sub},'_normxdim_anticausal_covnorm_NOVISWEIGHT_step1/occ_covnorm_temporal_tstep1_tau_8.mat',]);
...
%         load(['/Users/ranwang/Documents/writen_paper/NER2020/',subjs{sub},'_prearti/occ_cc_value.mat',]);
        eval([attr,'=',attr,';']);
        causal=false;
        ref = 'anticausal';%'anticausal';'causalpre';'causal';'percept';
        thepower=0.43;
        do_t_ds=false;
        Subj = ['NY',subjs{sub}];
        % get paths to all visualization files
...
%         plot_data_all_T1 = get_plot_data(paths, 'subj', 'lh');
    %     coord = T1;
        ...
        if strcmp(attr,'freq_formants_hamon')
            ...
            data_nt = att_noisetemporal(:,on_ttau:off_ttau,1,:,:,:);
            
            att_causal = eval([attr,'_causal']);
            data_c = att_causal(:,on:off,1,:,:);
            data_cpre = att_causal(:,1:4,1,:,:);
            att_anticausal = eval([attr,'_anticausal']);
            data_a = att_anticausal(:,on:off,1,:,:);
            att_percept = eval([attr,'_percept']);
                ...
                if plot_on_same_brain 
                    if sub == SUB
                        if ~return_curve
                            ...
                            [frames,frames_brain,err,tticks] = PlotAtt(Att1_cell, mask_cell,coord_cell,region_cell,causal,plot_data_all_mni,clrmap,att_cell_c,att_cell_cpre,att_cell_a,att_cell_p,ref,investreg);
                        end
%                         axes(haxis(sub2ind([col,row],dumm,1)));
%                         imshow(uint8(squeeze(frames)));
%                         axis off;
                ...
                if plot_on_same_brain 
                    if sub == SUB
                        if ~return_curve
                            ...
                            [frames2,frames2_brain,err2,tticks] = PlotAtt(Att2_cell, mask_cell,coord_cell,region_cell,causal,plot_data_all_mni,clrmap,att_cell_c,att_cell_cpre,att_cell_a,att_cell_p,ref,investreg);
                        end
%                         axes(haxis(sub2ind([col,row],dumm+1,1)));
%                         imshow(uint8(squeeze(frames)));
%                         axis off;
...
%     color_map = color_map_cb;
    ...
    coord_cell = {};
    region_cell = {};
%     subjs = {'717','742','749'}; %stoi
%     subjs = {'717','742','749','742','742'}; %mcd
    subjs = {'717','742','749','798'}; %cc
%     subjs = {'829'};
    cmax=1.2;
    scale = 0.85;
    cax=[-0.7,0.7];
...
%         load(['/Users/ranwang/Documents/writen_paper/NER2020/',subjs{sub},'_wpariental/occ_cc_temporal_8.mat',]);
        load(['/Users/ranwang/Documents/writen_paper/NER2020/entiregrid_',subjs{sub},'_normxdim_causal_covnorm_NOVISWEIGHT_step1/occ_cov_temporal_8.mat',]);
%         load(['/Users/ranwang/Documents/writen_paper/NER2020/entiregrid_',subjs{sub},'_normxdim_anticausal_percept_active_covnorm_NOVISWEIGHT_step1/occ_cov_temporal_8.mat',]);
%         load(['/Users/ranwang/greene/neural_decoding/code/cnn/ALAE/training_artifacts/entiregrid_',subjs{sub},'_han5amppowerloss_ecogfinetune_alphasup3_groupnormxdim_causal_/grad_cov_temporal_8.mat',]);
...
%         load(['/Users/ranwang/greene/neural_decoding/code/cnn/ALAE/training_artifacts/entiregrid_',subjs{sub},'_han5amppowerloss_ecogfinetune_alphasup3_groupnormxdim_anticausal_/grad_cov_temporal_8.mat',]);
%         load(['/Users/ranwang/greene/neural_decoding/code/cnn/ALAE/training_artifacts/entiregrid_',subjs{sub},'_han5amppowerloss_alphasup3_percept_singlebandwidth_fractive1500lim_bgnoiselearn_ecogfinetune_anticausal_passive_groupnormxdim_correctbaseline/grad_cov_temporal_8.mat',]);
...
%         coord = T1;
        coord = mni;
        aud = isaud(regions);
%         marcro_elec = zeros(15,15);
%         marcro_elec(1:2:end,1:2:end)=1;
%         mask = mask & marcro_elec;
%         mask_nonaud = mask & ~aud;
%         onset_diff = (onset-continuouse);
        if strcmp(attr,'freq_formants_hamon')
            att = eval(attr);
...
%                     Att1_causal = Att1./(abs(Att1)+abs(Att1_a));
%                     Att1_anticausal = Att1_a./(abs(Att1)+abs(Att1_a));
                end
...
%                         end
    ...
    %                         end
                            ...
                            if plot_on_same_brain 
                                if sub == SUB
%                                     [frames1,prob1,prob0] = VisualAtt(att_cell1, mask_cell, region_cell,coord_cell, plot_data_all_mni,color_map,cmax,true,false,annotation,bar_plot,true,1,10,0); % anticausal
%                                     [frames2,prob2,prob0] = VisualAtt(att_cell2, mask_cell, region_cell,coord_cell, plot_data_all_mni,color_map,cmax,true,false,annotation,bar_plot,true,1,10,1); % causal
                                    [frames1,prob1,prob0] = VisualAtt(att_cell1, mask_cell, region_cell,coord_cell, plot_data_all_mni,color_map,cmax,true,false,annotation,bar_plot); % anticausal
                                    [frames2,prob2,prob0] = VisualAtt(att_cell2, mask_cell, region_cell,coord_cell, plot_data_all_mni,color_map,cmax,true,false,annotation,bar_plot); % causal
%                                     xxx=att_cell1{1}; yyy=att_cell2{1};
%                                     [frames3,prob3,prob0] = VisualAtt({scale*(sign(xxx-yyy).*(abs(xxx-yyy)).^the_power)}, mask_cell, region_cell,coord_cell, plot_data_all_mni,color_map_div,1,true,true,annotation,bar_plot,false);
                                    if bar_plot
                                        [frames,prob1,prob0] = VisualAtt(Att1_cell, mask_cell, region_cell,coord_cell, plot_data_all_mni,color_map_div,1,true,true,annotation,bar_plot);
                                           ...
                                           [frames,prob1,prob0] = VisualAtt4normcontr(Att1_cell,att_cell1,att_cell2, mask_cell, region_cell,coord_cell, plot_data_all_mni,color_map_div,1,false,false,annotation);
                                            ...
                                            switch contrast
                                                case 'sin'
                                                    ...
                                                    frames={};Lcrtx = load('ch2_template_lh_pial.mat'); Lcrtx.tri = Lcrtx.faces;Lcrtx.vert = Lcrtx.coords; frames{1}=tripatch(Lcrtx, 'nofigure', da); shading interp;colormap(gca,color_map_div);material dull;lighting gouraud;light;axis off;viewangle = 'l';litebrain(viewangle,.8); set(gca,'CLim',[cax(1) cax(2)]); 
                                                case 'sqrt'
                                                    frames={};Lcrtx = load('ch2_template_lh_pial.mat'); Lcrtx.tri = Lcrtx.faces;Lcrtx.vert = Lcrtx.coords; frames{1}=tripatch(Lcrtx, 'nofigure', scale*(sign(prob1'-prob2').*(abs(prob1'-prob2')).^the_power)); shading interp;colormap(gca,color_map_div);material dull;lighting gouraud;light;axis off;viewangle = 'l';litebrain(viewangle,.8);;set(gca,'CLim',[cax(1) cax(2)]); 
                                                case 'minu'
                                                    frames={};Lcrtx = load('ch2_template_lh_pial.mat'); Lcrtx.tri = Lcrtx.faces;Lcrtx.vert = Lcrtx.coords; frames{1}=tripatch(Lcrtx, 'nofigure', prob1'-prob2'); shading interp;colormap(gca,color_map_div);material dull;lighting gouraud;light;axis off;viewangle = 'l';litebrain(viewangle,.8);;set(gca,'CLim',[cax(1) cax(2)]); 
                                            end
                                        end
