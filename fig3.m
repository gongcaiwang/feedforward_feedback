...
%                 point_alpha = max(min(abs(Att_frame(ind_active)),1),0);
        ...
        if gaussianplot
            ...
            color_map(1:int64(size(color_map,1)*0.03516/S.cax(2)),:)=1;
...
function [frames1,frames2,frames,Att1_cell] = attr_vis_contrast(attr,post,visdiff,postabs,max_att,plot_on_same_brain,above_median,temp,wreg,entire_period,avg_data,annotation,bar_plot,norm_contr_elec)    
    ...
    load('color_map_div.mat');
    load('color_map.mat');
    color_map_div_ = color_map_div_(end:-1:1,:);
    color_ind = value2colorind(ones(128,1),'hot',[0,1]); 
    color_ind = [color_ind,color_ind,color_ind];
    for ii=1:S_len
...
%         onregion = {'cSTG','mSTG','parstriangularis','parsopercularis','precentral','postcentral','inferiorparietal','supramarginal'};
        ...
        load(['fig3/entiregrid_',Ss{ii},'_normxdim_causal_covnorm_NOVISWEIGHT_step1/occ_cov_temporal_8.mat',]);
        for a= 1:length(attrs)
        ...
        load(['fig3/entiregrid_',Ss{ii},'_normxdim_anticausal_covnorm_NOVISWEIGHT_step1/occ_cov_temporal_8.mat',]);
        ...
        if plot_on_same_brain 
            if ii == S_len
                [frames1,prob1,prob0] = VisualAtt(att_cell1, mask_cell, region_cell,coord_cell, plot_data_all_mni,color_map_,cmax,true,false,annotation,bar_plot,true,[0,0.858]); title(gca,'figure 3b. Anti-causal contribution'); % anticausal
                [frames2,prob2,prob0] = VisualAtt(att_cell2, mask_cell, region_cell,coord_cell, plot_data_all_mni,color_map_,cmax,true,false,annotation,bar_plot,true,[0,0.858]); title(gca,'figure 3c. causal contibution'); % causal

                frames={};Lcrtx = load('ch2_template_lh_pial.mat'); Lcrtx.tri = Lcrtx.faces;Lcrtx.vert = Lcrtx.coords; frames{1}=tripatch(Lcrtx, 'nofigure', prob1'-prob2'); shading interp;colormap(gca,color_map_div_);material dull;lighting gouraud;light;axis off;viewangle = 'l';litebrain(viewangle,.8);set(gca,'CLim',[cax(1) cax(2)]);title(gca,'figure 3e. contribution contrast'); 
